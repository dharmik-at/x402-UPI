...
import db from "./db";
import { signReceipt, verifyReceipt } from "./utils/jwt";
import { startMockPsp } from "./mock_psp";
import { X402MCPServer } from "./mcp-server";
...
function createPaymentChallenge({ id, amount, currency, metadata, expiresAt }: any) {
...
async function handleMCPToolCall(toolName: string, args: any) {
...
async function createMCPPaymentChallenge(toolName: string, price: number, args: any, agentId?: string) {
  const payReqId = `mcp_${toolName}_${nanoid(8)}`;
...
// ✅ Verify MCP payment
async function verifyMCPPayment(paymentProof: string, toolName: string, expectedAmount: number, agentId?: string) {
  ...
  const consumed = db.prepare(`SELECT * FROM consumed_receipts WHERE receipt = ?`).get(paymentProof);
  if (consumed) {
    throw new Error('Payment receipt already used');
  }

  db.prepare(`INSERT INTO consumed_receipts (receipt, payreq_id, consumed_at) VALUES (?, ?, ?)`)
...
// 🔧 Execute MCP tools
async function executeMCPTool(toolName: string, args: any, context: any) {
  switch (toolName) {
...
function executeSummaryTool(args: any) {
...
function executeTranslateTool(args: any) {
...
function executeSentimentTool(args: any) {
...
function executeCodeTool(args: any) {
...
function executeResearchTool(args: any) {
...
app.post('/generate-summary', async (req, res) => {
  ...
  if (!auth) {
    ...
    const payReqId = `payreq_${nanoid(8)}`;
    const amount = 5.00;
  ...
  try {
    ...
    const payload: any = verifyReceipt(token) as any;
    
    // Confirm payreq status is PAID
    const row = db.prepare('SELECT * FROM payment_challenges WHERE id = ?').get(payload.sub) as any;
    if (!row || row.status !== 'PAID') {
      return res.status(403).json({ error: 'PaymentNotVerified' });
    }

    // Ensure receipt not consumed
    const consumed = db.prepare('SELECT * FROM consumed_receipts WHERE receipt = ?').get(token);
    if (consumed) {
...
app.post('/webhook/psp', (req, res) => {
  ...
  const payreq = db.prepare('SELECT * FROM payment_challenges WHERE id = ?').get(order_id);
  if (!payreq) {
  ...
  db.prepare('UPDATE payment_challenges SET status = ? WHERE id = ?').run('PAID', order_id);
  ...
  db.prepare('INSERT INTO issued_receipts (payreq_id, receipt, issued_at) VALUES (?, ?, ?)')
...
app.get('/payment/status', (req, res) => {
  const { payreq } = req.query as any;
  if (!payreq) return res.status(400).json({ error: 'missing payreq' });
  
  const row = db.prepare('SELECT status, expires_at FROM payment_challenges WHERE id = ?').get(payreq) as any;
  if (!row) return res.status(404).json({ error: 'unknown' });
...
app.get('/payment/receipt', (req, res) => {
  ...
  const rec = db.prepare('SELECT receipt FROM issued_receipts WHERE payreq_id = ? ORDER BY id DESC LIMIT 1').get(payreq) as any;
  if (!rec) return res.status(404).json({ error: 'receipt_not_ready' });
